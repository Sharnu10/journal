...
export const seedTasksTable = async () => {
  ...
  tasksList.forEach((task) => {
    db.run(
      insertQuery,
      [task.task, task.priority, task.category, task.createdDate],
      function (err) {
        if (err) {
          logger.info("seedTasksTable fun:- ", err.message);
        } else {
...
export const seedCardTable = async (
  tableName: string,
  columnNames: string,
  valuesLength: string
) => {
  ...
  cardList.forEach((card: any) => {
    ...
    db.run(
      ...
      function (err) {
        ...
        if (err) {
          logger.info("seedCardTable fun:- ", err.message);
        } else {
          logger.info("data inserted");
        }
      }
